      ...
      <div className="px-6 md:px-12 lg:px-24 py-20 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-16">
          <div className="md:col-span-1">
            <Link href="/" className="inline-flex items-center gap-3">
              ...
              <span className="text-lg font-semibold text-black">Quawaco</span>
            </Link>
          ...
          <div className="md:col-span-2">
            ...
            <form className="flex flex-col sm:flex-row items-center gap-3 w-full">
              ...
              <button
                type="submit"
                className="px-6 py-2 w-full md:w-auto rounded-md bg-blue-600 text-white hover:bg-blue-700 transition"
              >
                Subscribe
              </button>
            </form>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-sm text-gray-600">
          {sections.map(({ title, links }) => (
            <div key={title}>
              <h4 className="text-base font-semibold text-black mb-5">{title}</h4>
              <ul>
                {links.map(({ name, href }) => (
                  <li key={name} className="first:prose-a:pt-0 last:prose-a:pb-0">
                    <Link
                      href={href}
                      className="hover:text-blue-600 py-2 inline-block transition"
                    >
                      {name}
                    </Link>
                  </li>
